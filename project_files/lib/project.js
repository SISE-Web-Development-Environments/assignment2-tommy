...
function Game_Login() {
    ...
    if(newUserName == "" || newPassword == ""){
        ...
        document.getElementById("lblNameFinish").value = playerName;
        Show_Tab("game_page1");
    }else if(newUserName == "p" && newPassword == "p"){
        alert('Login success');
        playerName = "p";
        document.getElementById("lblName").value = playerName;
        document.getElementById("lblNameFinish").value = playerName;
        ...
        alert('Fail to login, try diffrent user name ore password');
    }
}
...
function Draw(Pos) {
    context.clearRect(0, 0, myCanvas.width, myCanvas.height); //clean board
...
function UpdatePosition() {
    ...
    if (time_elapsed == 0 || score == Max_points) {
        Finish_Game();
    } else if (lives == 0) {
        Finish_Game();
    } else
